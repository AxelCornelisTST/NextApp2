...
export async function findLaptopUserAction(prev: any, formData: FormData) {
    ...
    const result = await dbc.getRepository(LaptopUserEntity).find({
        select: {firstName: true, familyName: true, userID: true, klas: true},
...
export async function findLaptopUserWithDevicesAction(prev: any, formData: FormData) {
    const rawFormData = {
        name: formData.get('search'),
    }
    ...
    const result = await dbc.getRepository(LaptopUserEntity).find({
        relations: {laptop: true},
        select: {firstName: true, familyName: true, userID: true, klas: true, laptop: true},
        where: [{firstName: Like(`%${rawFormData.name}%`)}, {familyName: Like(`%${rawFormData.name}%`)}]
    });
    await dbc.destroy();
